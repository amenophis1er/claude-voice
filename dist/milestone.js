...
import { clampSpokenLength, sanitizeForSpeech } from "./sanitize.js";
/**
...
const stateFile = (s) => join(tmpdir(), `claude-voice-${s}.milestone`);
/**
 ...
 */
export function nextMilestone(session, text, intervalSeconds, now = Date.now()) {
    ...
    if (now - lastTs < intervalSeconds * 1000)
        return undefined;
    if (clean === lastText)
        return undefined;
    try {
...
export function clearMilestone(session) {
    try {
