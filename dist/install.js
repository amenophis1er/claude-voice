 ...
 */
function ensureDispatchPath() {
    if (!COMPILED)
        return resolve(HERE, "dispatch.ts");
...
export function install() {
    ...
    const cmdFor = (sub) => `node "${dispatch}" ${sub}`;
    ...
    for (const [event, sub, isAsync] of WIRING) {
        ...
        stripOurs(groups);
        const hook = { type: "command", command: cmdFor(sub) };
        if (isAsync) {
            hook.async = true;
            hook.timeout = 30;
        }
...
function stripOurs(groups) {
    for (let i = groups.length - 1; i >= 0; i--) {
        ...
        if (hooks.some((h) => /dispatch\.(ts|js)/.test(h.command ?? "")))
            groups.splice(i, 1);
    }
}
