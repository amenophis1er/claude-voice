...
import { APP_DIR } from "./install.js";
/**
...
export function buildShortcutPlist(command) {
...
function nodePath() {
    for (const p of ["/opt/homebrew/bin/node", "/usr/local/bin/node"]) {
        try {
            if (realpathSync(p) === realpathSync(process.execPath))
                return p;
        }
        catch {
...
export function createVoiceShortcut(kind = "recap") {
    const spec = SHORTCUTS[kind];
    if (!spec)
        throw new Error(`unknown shortcut "${kind}" — use: ${Object.keys(SHORTCUTS).join(", ")}`);
    if (platform() !== "darwin") {
