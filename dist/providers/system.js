...
async function linuxEspeak(input) {
    const audioFile = join(input.outDir, out("wav"));
    const args = ["-w", audioFile];
    if (input.voice)
        args.push("-v", input.voice);
    if (input.rate && input.rate !== 1)
        args.push("-s", String(Math.round(175 * input.rate)));
    args.push(input.text);
    try {
