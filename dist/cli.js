...
async function init() {
    ...
    const PRESETS = ["silent", "chimes", "summary", "verbose"];
    const presetIn = await ask("Verbosity: silent | chimes | summary | verbose", "summary");
    const preset = PRESETS.includes(presetIn) ? presetIn : "summary";
    if (preset !== presetIn)
        console.log(`  (unknown preset "${presetIn}" — using "summary")`);
    console.log("\nProviders:");
    for (const p of listProviders()) {
    ...
    if (doInstall.startsWith("y"))
        console.log("Installed →", install());
    else
        console.log("Skipped. Run `claude-voice install` later.");
