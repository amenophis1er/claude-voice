 ...
 */
export function sessionTabInfo(pid) {
    if (platform() !== "darwin")
        return undefined;
    let rows;
    try {
...
export function sessionTabFocused(info) {
    const nowMs = Date.now();
    if (focusCache && nowMs - focusCache.at < FOCUS_CACHE_MS)
...
let focusCache;
function queryTabFocus(info) {
    if (platform() !== "darwin")
        return undefined;
    const script = SCRIPTABLE[info.app];
    if (!script)
        return undefined; // terminal has no tty query bridge
    let frontApp;
    try {
        frontApp = execFileSync("osascript", ["-e", 'tell application "System Events" to get name of first application process whose frontmost is true'], { encoding: "utf8", timeout: 1000 }).trim();
    }
    catch {
        return undefined; // Automation permission not granted
    }
    if (frontApp !== info.app)
        return false; // terminal isn't the frontmost app
    let selectedTty;
    try {
 ...
 * Exported for tests. */
export function parseProcessTree(psOutput, pid) {
    const procs = new Map();
    ...
    for (let hops = 0; cur && hops < 32; hops++) {
        ...
        if (tty !== undefined && app !== undefined)
            break;
