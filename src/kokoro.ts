...
export interface ModelFile {
  ...
  url: string;
  approxMB: number;
  /** Sanity floor — a smaller file is a truncated download, not a model. */
  minBytes: number;
}
export const MODEL_VARIANTS: Record<"quantized" | "full", ModelFile> = {
...
export const VOICES_BIN: ModelFile = { file: "voices-v1.0.bin", url: `${RELEASE_BASE}/voices-v1.0.bin`, approxMB: 27, minBytes: 5_000_000 };
...
export function installedModel(): "quantized" | "full" | undefined {
  if (fileOk(join(MODELS_DIR, MODEL_VARIANTS.full.file), MODEL_VARIANTS.full.minBytes)) return "full";
  if (fileOk(join(MODELS_DIR, MODEL_VARIANTS.quantized.file), MODEL_VARIANTS.quantized.minBytes)) return "quantized";
  return undefined;
}

export function fileOk(path: string, minBytes: number): boolean {
  try {
...
export function pythonVersionOk(versionLine: string): boolean {
 ...
 */
export function findPythonPlan(): PythonPlan | undefined {
  try {
  ...
  for (const exe of ["python3.13", "python3.12", "python3.11", "python3.10", "python3"]) {
    try {
      ...
      if (pythonVersionOk(v)) return { kind: "venv", exe, detail: v };
    } catch {
      /* not on PATH */
    }
  }
...
export async function livePort(): Promise<number | undefined> {
  try {
    const port = Number(readFileSync(PORT_FILE, "utf8").trim());
    if (port > 0 && (await healthy(port))) return port;
  } catch {
    /* no port file */
  }
  return undefined;
...
export async function ensureServer(timeoutMs = 30_000): Promise<number> {
 ...
 */
function claimAndSpawn(): void {
  try {
    writeFileSync(LOCK_FILE, String(process.pid), { flag: "wx" });
  } catch {
    try {
      const holder = Number(readFileSync(LOCK_FILE, "utf8"));
      if (holder > 0 && alive(holder)) return; // someone else is spawning
...
export async function synthesizeWav(text: string, opts?: { voice?: string; speed?: number }): Promise<Buffer> {
  ...
  if (!res.ok) throw new Error(`kokoro server ${res.status}: ${await res.text()}`);
  return Buffer.from(await res.arrayBuffer());
}

/** All voice ids shipped in the voices file, from a live server. */
export async function serverVoices(): Promise<string[]> {
  const port = await ensureServer();
