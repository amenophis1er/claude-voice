...
import { playFile, playerCommand } from "./speak.ts";
import {
  DEFAULT_VOICE,
  DEPS_MARKER,
  KOKORO_DIR,
 ...
 */
const CURATED_VOICES: Array<{ id: string; hint: string }> = [
 ...
 */
export async function installKokoro(opts?: { writeConfig?: boolean }): Promise<InstallResult> {
  const writeConfig = opts?.writeConfig ?? true;
  if (writeConfig) ui.intro("kokoro", "local neural TTS — offline, free, no API key");
  ...
  if (!variant) {
    variant = await ui.select<"quantized" | "full">(
      "Model size",
      [
 ...
 */
export async function pickVoice(opts: { save: boolean }): Promise<string> {
  if (!isInstalled()) throw new Error("Kokoro is not installed — run `claude-voice kokoro install`");
  await ensureServer();

  const current = loadConfig().voice;
  const options: ui.SelectOption<string>[] = [
  ...
  let voice = await ui.select<string>(
    "Voice (each one speaks as you land on it)",
    options,
    initial,
    schedulePreview,
  );
  if (voice === "__all__") {
    ...
    voice = await ui.select<string>(
      ...
      Math.max(0, all.indexOf(current ?? DEFAULT_VOICE)),
      schedulePreview,
    );
  }
  stopPreview();
...
export async function uninstallKokoro(): Promise<void> {
...
export async function kokoroStatus(): Promise<string> {
  if (!isInstalled()) {
...
function schedulePreview(voice: string): void {
  if (voice === "__all__") return;
  clearTimeout(previewTimer);
  const seq = ++previewSeq;
  previewTimer = setTimeout(() => void playPreview(voice, seq), 250);
}

async function playPreview(voice: string, seq: number): Promise<void> {
  try {
    ...
    if (seq !== previewSeq) return; // user moved on while we synthesized
    previewProc?.kill();
...
function stopPreview(): void {
  ...
  previewSeq++;
  previewProc?.kill();
  for (const f of previewCache.values()) rmSync(f, { force: true });
  previewCache.clear();
}
...
async function downloadIfMissing(mf: ModelFile): Promise<void> {
  ...
  if (!res.ok || !res.body) throw new Error(`download failed (${res.status}): ${mf.url}`);
...
function updateConfig(mutate: (cfg: Record<string, unknown>) => void): void {
...
function run(cmd: string, args: string[]): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      if (code === 0) resolve();
      else reject(new Error(`${cmd} ${args[0]} failed (exit ${code}):\n${stderr.trim().split("\n").slice(-8).join("\n")}`));
    });
  });
}
